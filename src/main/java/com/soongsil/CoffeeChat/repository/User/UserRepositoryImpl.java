...
import com.soongsil.CoffeeChat.dto.QUserGetUpdateDto;
...
public class UserRepositoryImpl implements UserRepositoryCustom {
	...
	@Override
	public UserGetUpdateDto findUserInfoByUsername(String username) {
		...
		StringExpression roleExpression = new CaseBuilder()
				.when(mentor.isNotNull()).then("MENTOR")
				.when(mentee.isNotNull()).then("MENTEE")
				.otherwise("UNKNOWN");

		return queryFactory
				.select(new QUserGetUpdateDto(
						user.name,
						user.email,
						user.phoneNum,
						roleExpression,
